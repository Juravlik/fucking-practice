...
int main() {
	...
	int * mas_A = new int[n]; // ћассив возможных значений с.в. a_k
	...
	int * mas_AA = new int[n]; // ћассив с.в. в выборке
	
	int * mas_n_j = new int[n]; //ћассив, содержащий кол-во попаданий в определЄнный интверал
	for (int i = 0; i < n; i++) 
		mas_n_j[i] = 0;
	...
	double M_t = 0, M_v = 0;            // “еоретическое и выборочное мат. ожидание
	double D_t = 0, D_v = 0;            // “еоретическа€ и выборочна€ дисперси€
	...
	double * mas_E_j = new double[n]; // ћассив ожидаемых чисел попаданий в i-й интервал
	for(int i = 0; i < n; i++)
		mas_E_j[i] = n * mas_P[i];
